...
fig_notes <- "Notes: The result in this chart are obtained by summing up the total number of the five 'time-consistent' indicators that are true for each survey respondent."
...
fig_notes <- "Notes: This indicator measures the percentage of adults that have either channelled funds towards (a) the purchase of long-term physical assets, (b) made contributions to- or purchased- financial assets or (c) education for themselves or others."
...
indicators <- c("mfhi_f_secure", "mfhi_nf_secure", "mfhi_d_secure", "mfhi_ef_secure", "mfhi_i_p12mos_any", "mfhi_score_overall", "mfhi_score_hi")
...
indicators <- names(FA2024)[str_detect(names(FA2024), "mfhi_score_md2d|mfhi_score_risk|mfhi_score_inv")]
...
indicators <- names(FA2024)[str_detect(names(FA2024), "mfhi_score_hi|mfhi_score_med|mfhi_score_low")]
...
ggplot(data = FA2024, 
  ...
  scale_x_continuous(labels = scales::label_currency(prefix = "KSh")) +
  theme_custom() + 
...
# Defining regressions
  depvars <- c("mfhi_score_hi")
  depvar_labels <- INDICATORS[depvars]
  
  #"resp_edu_group2_pri", "resp_edu_group2_sec", "resp_edu_group2_trt")
  
  maineffects_base <- c("hh_urbrur_Urban", "hh_size_all_c", "resp_gender_group_wmn", "resp_age_yrs_c_5", "resp_income_c_5000", "resp_income_var")
   maineffects_all <- c("hh_urbrur_Urban", "hh_size_all_c", "resp_gender_group_wmn", "resp_age_yrs_c_5", "resp_income_c_5000", "resp_income_var", "resp_live_group_empl", "resp_live_trnsf_in", "fb_financial_plan", "fb_gambling", "fb_emergency_savings", "fl_literacy_all",  "sc_ff_finhelp")
...
fig_data <- prep_reg_data(prep_reg_factors(ests, factor_params, include_valuelabel = FALSE)) %>% mutate(effect_label = fct_rev(effect_label))
 ...
 labels <- list(
      title = "Predictors of financial health",
      subtitle = NULL,
...
facets <- list(type = "wrap", rows = "model_type", cols = "model_type", scales = "free_x", space = "free", drop_row_label = TRUE) 
    
fig_regests(fig_data, labels, facets = facets, barparams, scales, effect_desc = "The effect of each predictor on the probability of having a high financial health score (> 0.6)", coord_flip = TRUE)
