...
```{r, fig.width = 12, fig.asp = 0.55}
#| label: fig-mfhi_score_cats_bylive
#| echo: false
#| warning: false
#| fig-cap: "MFHI score categories, by livelihood"
#| cap-location: margin

fig_notes <- ""

indicators <- names(FA2024)[str_detect(names(FA2024), "mfhi_score_hi_c|mfhi_score_med_c|mfhi_score_low_c")]
groups_custom <- GROUPS[c("resp_live_group_fct")]
...
labels <- list(
  title = "Multi-dimensional financial health index score categories, by main source of income",
  subtitle =  "Adults (% 18+)",
  yax_ti = NULL,
  xax_ti = NULL,
  caption = str_wrap(paste(SOURCE, fig_notes), CAP_WRAP)
...
ggplot(data = FA2024, 
  ...
  labs(
    ...
    caption = str_wrap("Source: Author's calculations based on 2024 FinAccess survey.", CAP_WRAP)
  ) + 
...
fig_notes <- "Notes: The results in this figure are based on a linear regression model that measures the influence of the size component indicators of the MFHI on how adults perceive different aspects of their financial lives. The figures show the effect of each outcome on the probability of a positive perception of their financial situation (in percentage points). "
...
scales <- list(
  y = list(limits = c(miny,maxy), position = "left", nbreaks = 5), 
...
model <- lm(mfhi_score_hi_c ~ resp_income_c_5000 + resp_income_var + resp_live_group_empl + resp_edu_group2_sec + resp_edu_group2_trt, data = FA2024 %>% filter(resp_age_group2_fct == "[25-65)"))
...
ggplot(data = FA2024 %>% filter(resp_age_group2_fct == "[25-65)"), 
  ...
  labs(
    ...
    caption = str_wrap(paste(SOURCE, "Notes: X-axis is truncated at Ksh 75,000, predicttions take into account all data points"), CAP_WRAP)
  ) + 
...
# Defining regressions
  ...
  maineffects_base <- c("hh_urbrur_Urban", "hh_size_all_c", "resp_gender_group_wmn", "resp_age_yrs_c_5", "resp_income_c_5000", "resp_income_var", "resp_live_group_empl",  "resp_edu_group2_pri", "resp_edu_group2_trt")
   maineffects_all <- c("hh_urbrur_Urban", "hh_size_all_c", "resp_gender_group_wmn", "resp_age_yrs_c_5", "resp_income_c_5000", "resp_income_var", "resp_live_group_empl",  "resp_edu_group2_pri", "resp_edu_group2_trt", "fb_financial_plan", "fb_emergency_savings", "fl_literacy_all",  "sc_ff_finhelp")
      maineffects_all2 <- c("hh_urbrur_Urban", "hh_size_all_c", "resp_gender_group_wmn", "resp_age_yrs_c_5", "resp_income_c_5000", "resp_income_var", "resp_live_group_empl", "resp_edu_group2_pri", "resp_edu_group2_trt", "fb_financial_plan", "fb_emergency_savings", "fl_literacy_all",  "sc_ff_finhelp", "fin_reg_mobilemoney", "fin_reg_mobilebanking", "fin_reg_bankmfb", "fin_reg_sacco", "fin_reg_chama")
...
# Defining regressions
  ...
  depvar_labels <- INDICATORS[depvars]
  maineffects <- c("hh_urbrur_Urban", "hh_size_all_c", "resp_gender_group_wmn", "resp_age_yrs_c_5", "resp_income_c_5000", "resp_income_var", "resp_live_group_empl", "resp_edu_group2_pri", "resp_edu_group2_trt", "fb_financial_plan", "fb_emergency_savings", "fl_literacy_all",  "sc_ff_finhelp", "fin_reg_mobilemoney", "fin_reg_mobilebanking", "fin_reg_bankmfb", "fin_reg_sacco", "fin_reg_chama")
...
# Defining regressions
  ...
  confounds <- c("resp_income_c_5000", "resp_income_var", "resp_live_group_empl", "resp_edu_group2_pri", "resp_edu_group2_trt")
  effect_labels <- c("(Intercept)" = "Baseline", INDICATORS[maineffects])
